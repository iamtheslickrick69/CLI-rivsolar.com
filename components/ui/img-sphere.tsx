...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    const positions = imagePositions.map((pos, index) => {
      ...
      const distancePenalty = isPoleImage ? 0.3 : 0.5;
      const centerScale = Math.max(0.4, 1 - distanceRatio * distancePenalty);

      // Dramatic Z-depth scaling - front images 3x bigger than back
      const depthScale = (worldPos.z + actualSphereRadius) / (2 * actualSphereRadius);
      // Front: up to 1.5x, Back: down to 0.3x for dramatic 3D
      const scale = centerScale * Math.max(0.25, 0.3 + depthScale * 1.2);
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden border-[3px] border-white/50 shadow-lg">
          <img
            src={image.src}
            alt={image.alt}
