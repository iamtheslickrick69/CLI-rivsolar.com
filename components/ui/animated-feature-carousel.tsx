...
function FeatureCard({ children, step, steps }: { children: React.ReactNode; step: number; steps: readonly Step[] }) {
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-[24px] border border-zinc-800 bg-zinc-900/80 transition-colors duration-300">
        <div className="p-6 md:p-10 min-h-[450px] w-full">
          ...
          return (
            <motion.li key={step.name} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className="relative">
              <button
                type="button"
                className={cn(
                  "group flex items-center gap-2.5 rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-purple-500 focus-visible:ring-offset-zinc-900",
                  isCurrent
...
const defaultClasses = {
  img: "rounded-xl border border-zinc-700 shadow-2xl shadow-black/30",
